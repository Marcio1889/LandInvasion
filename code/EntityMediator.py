...
class EntityMediator:
    ...
    @staticmethod
    def __verify_collision_entity(ent1, ent2):  # Verificando as colisões entre tiros e naves
    ...
    def __offer_score(enemy: Enemy, entity_list: list[Entity]):
        if enemy.last_dmg == 'Player1Power':
    ...
    def verify_collision(entity_list: list[Entity]):
        for i in range(len(entity_list)):
            ...
            EntityMediator.verify_collision_window(entity1)  # Método agora acessível
    ...
    def verify_health(entity_list: list[Entity]):
        for ent in entity_list:
            ...
            if ent.health <= 0:
                if isinstance(ent, Enemy):
                    EntityMediator.__offer_score(ent, entity_list)
